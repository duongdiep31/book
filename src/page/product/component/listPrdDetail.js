...
import { get, prdCategoryApi } from "../../../api/product"
...
const CListPrdDetail = () => {
  ...
  useEffect(() => {
    const getList = async () => {
     const {data} =  await prdCategoryApi(id,page)
     setdatas(data)
    }
    getList()
  }, [id,page])
  ...
  const showing = () => {
    ...
    return (
      <p className="text-small text-muted mb-0"> {t('shop.show', {start: start, end: end, total: datas.total})}</p>
    )
  }
  return (
    <React.Fragment>
      ...
      <div className="col-lg-9 order-1 order-lg-2 mb-5 mb-lg-0">
        ...
        <nav aria-label="Page navigation example">
          <ReactPaginate
            ...
            nextLabel={'>>'}
            breakLabel={'...'}
            pageCount={Math.ceil(datas.total/ page.limit)}
